...
export default function RouteStopEdit() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (resData) {
        notify(translate("resources.routeStop.validation.updateSuccess"), {
          type: "success",
        });
        redirect(`/polyline/edit?routeId=${currentRouteId}`);
      }
    } catch (error) {
      console.error("error", error);
      notify(
        `${translate("resources.routeStop.validation.updateError")}: ${error}`,
        { type: "error" }
      );
    }
  };
